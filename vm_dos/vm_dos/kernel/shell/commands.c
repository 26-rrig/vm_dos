...
{
    con_write_at(25, 8, "Commands: help, clear, theme, ls, cat, echo, panel, progress, toast");
    return 0;
}
...
static int cmd_theme(int a, const char **v)
{
    if (a < 2)
    {
        con_write_at(25, 12, "Usage: theme <default|night|ocean>");
        return 1;
    }
    if (!strcmp(v[1], "default"))
        theme_set(&THEME_DEFAULT);
    else if (!strcmp(v[1], "night"))
        theme_set(&THEME_NIGHT);
    else if (!strcmp(v[1], "ocean"))
        theme_set(&THEME_OCEAN);
    ui_draw_chrome();
    toast_show("Theme applied", 300);
    return 0;
}
