...
void ui_draw_fullscreen_shell(void)
{
    ...
    for (int x = 0; x < VGA_WIDTH; x++)
        con_putc_at(x, 24, ' ', t->dim, t->accent);
    con_write_at(2, 24, " • Enter: exit fullscreen shell. • help: commands. ");

    // Main shell box
    con_box(0, 1, 79, 22, t->fg, t->bg, " SYSTEM: vm_dos Shell ");
...
void ui_draw_chrome(void)
{
    ...
    // Right info panel
    con_box(58, 1, 79, 22, t->fg, t->bg, " Info ");
